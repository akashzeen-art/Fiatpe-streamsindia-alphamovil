      ...
      <div className="w-full max-w-2xl mx-auto px-6 sm:px-12 py-16">
        ...
        {isLoggedIn ? (
          ...
          >
            ...
            {/* Active badge */}
            {plan && (
              <div className="flex items-center gap-3 bg-purple-950/40 border border-purple-500/20 rounded-xl px-5 py-4">
                <CheckCircle size={18} className="text-purple-400 shrink-0" />
                <p className="text-purple-300 text-sm">Your subscription is active. Enjoy unlimited thriller, crime &amp; drama content.</p>
              </div>
            )}

            <div className="pt-4 border-t border-white/10 text-center">
              <p className="text-white/50 text-sm mb-6">Explore our latest originals and web series.</p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <button
                  onClick={() => navigate('/')}
                  className="px-6 py-3 btn-neon text-white font-orbitron font-bold uppercase text-xs rounded-xl transition-all hover:scale-105 active:scale-95 shadow-lg shadow-purple-500/20 tracking-widest"
                >
                  Browse Content
                </button>
                <button
          ...
          >
            ...
            <button
              onClick={openFlow}
              className="w-full btn-neon text-white font-bebas font-bold py-4 rounded-xl text-xl tracking-widest transition-all hover:scale-[1.02] active:scale-95 shadow-lg shadow-purple-500/20"
            >
              Sign In / Subscribe
            </button>
            ...
            <p className="text-center text-gray-600 text-xs font-orbitron mt-6 leading-relaxed">
              New here?{' '}
              <button onClick={openFlow} className="text-purple-400 hover:underline">
                Subscribe now
          ...
          >
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 'mobile' && (
                <motion.div
                  key="mobile"
                  className="relative glass-panel rounded-2xl shadow-2xl p-8 w-full max-w-md mx-4 z-10"
                ...
                >
                  ...
                  <form onSubmit={handleMobileSubmit}>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex rounded-xl border border-white/15 overflow-hidden bg-white/5 focus-within:ring-2 focus-within:ring-purple-500 focus-within:border-transparent transition-all">
                    ...
                    <button
                      type="submit"
                      className="w-full btn-neon text-white font-bebas font-bold py-3.5 rounded-xl text-lg tracking-widest transition-all hover:scale-105 active:scale-95 shadow-lg shadow-purple-500/20"
                    >
                      Continue
                    </button>
                  </form>
                ...
                >
                  ...
                  <form onSubmit={handlePlanSubmit}>
                    ...
                    <p className="text-center mb-5 text-gray-500 text-xs font-orbitron uppercase tracking-widest">
                      Mobile: <span className="text-purple-400 font-bold">+91 {flowMobile}</span>
                    </p>
                      ...
                      {PLANS.map(p => (
                        ...
                        >
                          <input
                            ...
                            className="mt-0.5 mr-3 accent-purple-500"
                          />
                          <div className="flex-1">
                            <div className="flex items-center flex-wrap gap-2">
                              <span className="font-bebas text-lg text-white tracking-wide">{p.label}</span>
                              <span className="font-bold text-purple-400">{p.price}</span>
...
function InfoRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4 py-2 border-b border-white/5 last:border-0">
      ...
      <span className={`text-sm font-medium ${highlight ? 'text-purple-400' : 'text-gray-200'}`}>{value}</span>
    </div>
  );
}
